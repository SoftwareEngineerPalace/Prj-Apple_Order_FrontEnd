...
const App = () => {
    ...
    const getLineItemList = React.useCallback(
        async (orderId: string) => {
            ...
            const { data } = await fetch(`http://${hostname}:3001/order/getLineItemByOrderId?${paramStr}`).then((response) => response.json());
            ...
            <Card>
                <Table
                    ...
                    expandable={{
                        ...
                        expandIcon: ({ expanded, onExpand, record }) => (expanded ? <div onClick={(e) => onExpand(record, e)} /> : <div onClick={(e) => onExpand(record, e)} />),
                    }}
